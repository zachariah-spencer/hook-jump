  ...
  def calc_collisions
    ...
    if @player.carried_by_eagle
      rocks_hit = state.rock_manager.rocks.select do |rock|
        !rock.break_started_tick && @player.intersect_rect?(rock)
      end
    ...
    rocks_hit = []
    state.rock_manager.rocks.each do |rock|
      rocks_hit << rock if !rock.break_started_tick && @player.hook.intersect_rect?(rock)
    end
